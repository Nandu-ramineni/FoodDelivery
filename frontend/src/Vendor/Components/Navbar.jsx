...
const Navbar = () => {
    ...
    const { account, setAccount, firmName, setFirmName } = useContext(DataContext);
    const [dropdownVisible, setDropdownVisible] = useState(false);

    useEffect(() => {
        const storedAccount = localStorage.getItem('vendorUsername');
    ...
    return (
        <div className="w-full flex justify-between items-center py-6 px-4">
            {account ?(
                <div>
                <h2 className="text-lg">Hi👋 {firmName}</h2>
            </div>
            ):(
                <p></p>
            )}
